...
com_long %>%
    ...
    ggplot(., aes(x = water_year, y = val, color = site_code)) +
    #geom_point()+
    geom_abline(aes(slope=trend, intercept = 0, color = site_code))+
    theme(legend.position = 'none')
...
context <- com %>%
    ...
    mutate(domain = case_when(domain == 'neon' ~ paste0(domain, '_', site_code),
                              domain != 'neon' ~ domain),
           runoff_ratio = flow/precip) %>% # disaggregate neon sites since its national scale
    group_by(domain) %>%

...
#         theme_few()
# 
#     quietly(ggsave(plot = diag, filename = here('data_working', 'diag_plots', 'main_analysis', i, paste0(i,'_',j,'.png')),
#            create.dir = T))
# 
# 
...
write_csv(trends_flagged, file = here('data_working', 'hydro_climate_trends_flagged.csv'))


full<- as.integer(length(unique(trends_flagged$var)))
half <- as.integer(length(unique(trends_flagged$var))/2 )
```
...
```{r}
trends_flagged %>%
    filter(var %in% unique(trends_flagged$var)[1:half]) %>%
...
```{r}
trends_flagged %>%
    filter(var %in% unique(trends_flagged$var)[half+1:full]) %>%
ggplot(., aes(x = trend, fill = flag))+
    geom_density()+
    facet_wrap(~var, scales = 'free')+
    theme_few()+

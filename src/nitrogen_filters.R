...
# there is a minimum of 2 observations per month and 10 months
...
n_month_years <- n_monthly_vwm %>%
    ...
    mutate(flag_10 = case_when(`10` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_11 = case_when(`11` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_12 = case_when(`12` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_1 = case_when(`1` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_2 = case_when(`2` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_3 = case_when(`3` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_4 = case_when(`4` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_5 = case_when(`5` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_6 = case_when(`6` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_7 = case_when(`7` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_8 = case_when(`8` >= 2 ~ 1,
                               TRUE ~ 0),
           flag_9 = case_when(`9` >= 2 ~ 1,
                               TRUE ~ 0)) %>%
...
# This drops us from 8495 site-years to 2602 site-years or
# a 70% loss of data.
...
# Using the monthly dataset, let's count observations per month
# and trim down to >2 obs. per month.
n_mos_trim <- n_monthly_vwm %>%
    select(site_code, analyte, year, month, n_obs) %>%
    filter(n_obs >= 2)
# down to 58282 site-months of data, 26% loss of data
...
n_moyrs_trim <- n_moyears %>%
    # First filter = 10 years minimum
    filter(years >= 10) %>% # 44648 site-months of data remaining
    # And perform calculations for data coverage
    mutate(duration = end - start) %>%
    mutate(coverage = years/duration) %>%
    # Second filter = 60% of data present
    filter(coverage > 0.6) %>%
    select(site_code, analyte, year, month) # 34406 site-months remaining

# Finally merge with annual vwm data.
n_monthly_vwm_filtered <- left_join(n_moyrs_trim, n_monthly_vwm,
                                   by = c("site_code", "analyte",
                                          "year", "month"))
# This drops us from 79244 site-months to 34406 site-months or
# a 57% loss of data.
